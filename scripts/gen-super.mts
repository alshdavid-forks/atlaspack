...
const __output = path.join(
  __cwd,
  `atlaspack-${process.platform}-${process.arch}`,
);
const tarball = `atlaspack-${process.platform}-${process.arch}.tar.xz`;

const toCopy = ['node_modules', 'packages'];
...
void (async function main() {
  ...
  fs.cpSync(
    path.join(__root, 'node_modules'),
    path.join(__output, 'node_modules'),
    {recursive: true},
  );
  fs.rmSync(path.join(__root, 'node_modules', '.bin'), {
    recursive: true,
    force: true,
  });
  ...
  for (const packageJson of glob.sync(
    path.join(__output, 'node_modules', '**', '*', 'package.json'),
    {cwd: __root},
  )) {
    try {
  ...
  fs.writeFileSync(
    path.join(__output, 'package.json'),
  ...
  for (const namespace of fs.readdirSync(path.join(__root, 'packages'))) {
    // namespace collision with package @atlaspack/utils
    if (namespace === 'utils') continue;

    if (fs.existsSync(path.join(__output, namespace))) {
      fs.rmSync(path.join(__output, namespace), {recursive: true, force: true});
    }
    fs.mkdirSync(path.join(__output, namespace));

    for (const packageName of fs.readdirSync(
      path.join(__root, 'packages', namespace),
    )) {
      let packageJson = path.join(
        __root,
        'packages',
        namespace,
        packageName,
      ...
      // @ts-expect-error
      let nmPackageJson = nodeModule.findPackageJSON(
        name,
        path.join(__output, 'package.json'),
      );
  ...
  for (const packageName of fs.readdirSync(path.join(__output, 'core'))) {
    ...
    fs.symlinkSync(
      path.join('.', 'core', packageName),
      path.join(__output, packageName),
    );
  }
  // namespace collision
  fs.symlinkSync(
    path.join('.', 'node_modules', '@atlaspack', 'events'),
    path.join(__output, 'events'),
  );
